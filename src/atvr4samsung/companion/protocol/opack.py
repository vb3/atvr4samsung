"""OPACK serialization. Origin: pyatv v0.18.0 (MIT), adapted; first-party here.
...
def _pack(data, object_list):
    ...
    # OPACK encodes repeated objects by reference to the earlier object-table entry.
...
def _unpack(data, object_list):
    ...
    if data[0] == 0x01:
        ...
        # Absolute time is preserved as its raw 64-bit payload; packing remains unsupported.
        ...
        output = []
        ptr = data[1:]
        if count == 0xF:  # 0x03-terminated list
            while ptr[0] != 0x03:
        ...
        output = {}
        ptr = data[1:]
        if count == 0xF:  # 0x03-terminated dict
            while ptr[0] != 0x03:
