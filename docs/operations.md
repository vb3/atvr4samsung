...
| **Service refuses to start: "server-identity.json … is corrupt"** | The persisted Apple-TV identity is unreadable; the bridge fails closed rather than silently minting a **new** identity (which would force the iPhone to re-pair and reopen PIN bootstrap pairing). Run `atvr4samsung unpair --reset-identity` to regenerate it deliberately (you'll re-pair once), or restore/remove the file. State writes are now atomic + fsynced, so this should no longer happen from a power loss mid-write. |
